...
class Callback extends \Nette\Object implements \Nette\Diagnostics\IBarPanel
{
	...
	{
		$httpRequest = $this->container->getService('Nette\Web\IHttpRequest');
		...
		$cacheStorage = $this->container->getService('Nette\Caching\IStorage');
		$this->addCallback('--cache', "Invalidate cache", function() use($cacheStorage) {
			$cacheStorage->clean(array(\Nette\Caching\Cache::ALL => TRUE));
		});
		
		$robotLoader = $this->container->getService('Nette\Loaders\RobotLoader');
	...
	public function getPanel()
	{
		...
		$absoluteUrl = $this->container->getService('Nette\Web\IHttpRequest')->url->absoluteUrl;
		ob_start();
