...
class AuthorizatorTest extends \PHPUnit_Framework_TestCase
{
	public function testParseAnnotation()
	{
		$this->assertEquals(
			array(Authorizator::ROLE => "admin", Authorizator::RESOURCE => "foo", Authorizator::PRIVILEGE => "bar"), 
			Authorizator::parseAnnotations('NellaTests\Security\Foo', 'bar1'), 
			"::parseAnnotations allowed"
		);
		
		$this->assertEquals(
			array(Authorizator::ROLE => "admin", Authorizator::RESOURCE => "foo", Authorizator::PRIVILEGE => "bar"), 
			Authorizator::parseAnnotations('NellaTests\Security\Foo', 'bar2'), 
		...
		$this->assertEquals(
			...
			Authorizator::parseAnnotations('NellaTests\Security\Foo', 'bar3'), 
